object Versions {
    const val gradle = "4.2.0-alpha13" // Update root/buildSrc/build.gradle.kts as well
    const val kotlin = "1.4.10" // Update root/buildSrc/build.gradle.kts as well

    const val coreKtx = "1.3.1"
    const val appCompat = "1.2.0"
    const val material = "1.2.1"
    const val constraintLayout = "2.0.1"
    const val axJunit4 = "1.1.2"
    const val axEspresso = "3.3.0"
    const val assertK = "0.23"
    const val mockWebServer = "4.9.0"
    const val playCore = "1.8.0"
    const val playCoreKtx = "1.8.1"
    const val fragment = "1.3.0-alpha08"
    const val mockk = "1.10.0"
    const val nav = "2.3.0"
    const val junit5Plugin = "1.6.2.0"
    const val junit5 = "5.6.2"
    const val spek = "2.0.13"
    const val lifecycle = "2.2.0"
    const val arch = "2.1.0"
    const val room = "2.2.5"
    const val retrofit = "2.9.0"
    const val coroutineTest = "1.3.9"
}